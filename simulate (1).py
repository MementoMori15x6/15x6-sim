...
def compute_parasitism_proxy(scores):
    """Tuned for ~43% on USA baseline (D1 ≈ -3.4, G1 ≈ 4.0)."""
    d1 = scores[9]   # D1 parasitism
    g1 = scores[15]  # G1 cheater detection
    
    base_from_d1 = max(0, -d1 * 11.8)          
    detection_penalty = max(0, (5 - g1) * 5)   
...
def apply_shock(scores, shock_type="none"):
    ...
    if shock_type == "rigidity_collapse":
        ...
        shocked[:18] -= 3.0
        gov_indices = list(range(18, 35))
        shocked[gov_indices] -= 4.0
        shocked[9] = -6.5
        shocked[15] = 1.0
        return shocked
    
    raise ValueError(f"Unknown shock: {shock_type}")
