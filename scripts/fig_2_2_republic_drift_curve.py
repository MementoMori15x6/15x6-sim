# -*- coding: utf-8 -*-
"""
fig_2_2_republic_drift_curve.py

Purpose: Plot drift curve for Roman Republic phase transition (Chapter 2).
...
# Data points for Late Republic drift (approximate from our earlier runs)
years_bce = [-133, -100, -50, -27]
export_bce = [7.8, 6.0, 3.5, 1.1]
paras_bce = [8.0, 12.0, 18.0, 20.0]

fig, ax = plt.subplots(figsize=(12, 6))
ax.plot(years_bce, export_bce, label='Entropy Export Ratio', linewidth=3, color='blue')
ax.plot(years_bce, paras_bce, label='Rule-13 Parasitism %', linewidth=3, color='orange')
...
ax.axvline(-27, color='green', ls='--', linewidth=2, label='Principate Transition 27 BCE')

ax.set_xlabel('Year BCE')
ax.set_ylabel('Value')
ax.set_title('Republic Drift Curve: Expansion Buffer Delays Collapse')
ax.legend(loc='upper right')
ax.grid(True, alpha=0.3)
ax.invert_xaxis()  # BCE left to right decreasing
plt.tight_layout()
plt.savefig("fig_2_2_republic_drift_curve.png", dpi=300, bbox_inches="tight")
plt.show()
