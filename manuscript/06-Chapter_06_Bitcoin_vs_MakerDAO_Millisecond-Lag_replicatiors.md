...
[Unchanged from prior draft — keeps the ~20.6% → ~28.1% result, edging toward the 30% line.]
=========
Bitcoin achieves biological-grade Rule-13 fidelity by externalizing suppression cost to thermodynamics (energy), mirroring ant worker policing. The trade-off is constrained variation bandwidth (C2 ≈ 5), prioritizing persistence over rapid adaptation—essentially "freezing" its 15×6 lattice in a high-metabolism / high-suppression hybrid, where the "queen" is replaced by a mathematical constant.

MakerDAO exploits millisecond-lag advantages for metabolic flexibility (higher C2, niche construction), but inherits VOC-like governance drift: economic weights centralize → parasitism proxy rises → entropy export cost increases. This "physics vs. plutocracy" thesis explains the dynamics—Bitcoin offloads suppression to energy (low internal cost), while MakerDAO internalizes it to governance (capital), risking a "Roman" plunge from over-centralization.
