...
The 35-metric compass (Chapter 2) converts public evidence into X/Y coordinates, zone percentages, Rule-13 parasitism proxy, and a longevity placeholder. LLMs synthesize sources and output parseable scores fast.
...
Score the Bitcoin protocol (2009–2025, current dynamics) as a replicating system (public sources: CoinMetrics, Blockchain.com, Chainalysis, academic PoW papers).

Output strictly in this order (no extra prose):
1. CSV row of 35 raw scores (-10 to +10 integers, comma-separated, no header)
2. X,Y coordinates (X = mean(first 18)/10 × 1.2; Y = mean(last 17)/10 × 2.5)
3. Zone percentages (inverse distance to 10 attractors, power-law **10, normalized to 100%)
4. Dominant interaction per rule row (+/+/–/0 etc.)
5. Rule-13 parasitism proxy (max(0, 50 - (mean(last 17)/10 * 50)))
6. Longevity placeholder (toy heuristic: 100 / (1 + proxy/100) × (1 + mutualism/competition avg rows 6–8))
...
Output exactly:
1. CSV row of 35 raw scores (-10 to +10 integers, comma-separated)
2. X,Y coordinates
3. Zone percentages
4. Dominant interaction per rule row
5. Rule-13 parasitism proxy
6. Longevity placeholder
7. Brief analytical summary (2–3 sentences)
...
- Where It Stands (1–2 sentences: phase-space position, dominant zone)
