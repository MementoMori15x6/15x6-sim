...
def mc_aligned_proxy(d1, g1, f2, rigidity=0.0):
    f2_effect = (f2 * -0.2) if f2 >= 0 else (f2 * 0.35)
    raw = 40.5 + (d1 * 0.9) + (g1 * -2.4) + f2_effect + (rigidity * 38.0)
    bounded = np.clip(80 / (1 + np.exp(-(raw - 41.5) / 8.0)), 0, 80)
    return bounded
